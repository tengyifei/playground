...
def step_duration_from_latest_profile(
    profile_dir: str, ignore_gaps=False, verbose=True
) -> float:
    ...
    return analyze_step_duration(
        newest_profile, ignore_gaps=ignore_gaps, verbose=verbose
    )


def analyze_step_duration(file_path: str, ignore_gaps=False, verbose=True) -> float:
    xspace = XSpace()
    ...
    return analyze_step_duration_from_pb(
        xspace, ignore_gaps=ignore_gaps, verbose=verbose
    )


def analyze_step_duration_from_pb(
    xspace: XSpace, ignore_gaps=False, verbose=True
) -> float:
    offsets = []
    unique_names = set()
    all_durations = []
    ...
    for plane in xspace.planes:
        ...
        for line in plane.lines:
            ...
            for event in line.events:
                ...
                all_durations.append(event.duration_ps)
                if verbose:
                    print(
    ...
    if ignore_gaps:
        durations = [d / 1e12 for d in all_durations]
    else:
    ...
    if verbose:
        print(
            f"Got {event_count} intervals for event '{inferred_event_name}'",
            file=sys.stderr,
        )
